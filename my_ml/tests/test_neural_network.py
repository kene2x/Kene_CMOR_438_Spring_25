import numpy as np
import pytest
from my_ml.neural_network import DenseNetwork, activation, d_activation, compute_loss

def test_sigmoid():
    # Test sigmoid function
    assert activation(0) == 0.5
    assert activation(100) > 0.99  # Very large positive number
    assert activation(-100) < 0.01  # Very large negative number
...
def test_d_sigmoid():
    ...
    assert d_activation(0) == 0.25  # Maximum value at x=0
    x = np.array([-1, 0, 1])
    result = d_activation(x)
...
def test_mse():
    ...
    error = compute_loss(net.W, net.B, X, y)
